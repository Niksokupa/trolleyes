...
public class UsuarioDao {
	...
	public UsuarioBean create(UsuarioBean oUsuarioBean) throws Exception {
		String strSQL = "INSERT INTO " + ob + " (`id`, `dni`, `nombre`, `ape1`,`ape2`,`login`,`pass`,`id_tipoUsuario`) "
                        + "VALUES (NULL,?,?,?,?,?,?,?); ";
		ResultSet oResultSet = null;
		PreparedStatement oPreparedStatement = null;
		try {
	...
	public int update(UsuarioBean oUsuarioBean) throws Exception {
		int iResult = 0;
		String strSQL = "UPDATE " + ob + " SET codigo = ?, desc = ?, existencias = ?, precio = ?, foto = ?, id_tipoUsuario = ? "
                        + "WHERE id = ?;";
